...
public class FButton extends Button implements View.OnTouchListener {
    ...
    private void parseAttrs(Context context, AttributeSet attrs) {
        ...
        for (int i = 0; i < typedArray.getIndexCount(); i++) {
            ...
            } else if(attr == R.styleable.FButton_customBackground){
                mCustomBackground = typedArray.getBoolean(attr, true);
            } else if(attr == R.styleable.FButton_facebookBtn){
                mFacebookBtn = typedArray.getBoolean(attr, false);
            }
        }
        typedArray.recycle();
    ...
    private void updateBackground(Drawable background) {
        ...
        if (Build.VERSION.SDK_INT >= 16) {
            if(mCustomBackground) {
                this.setBackground(background);
                //this.setBackground(getResources().getDrawable(R.drawable.button));
            }else {
                if(mFacebookBtn){
                    this.setBackground(getResources().getDrawable(R.drawable.button));
                }else {
                    this.setBackground(getResources().getDrawable(R.drawable.boder));
                }
            }
        } else {
            if(mCustomBackground) {
                this.setBackgroundDrawable(background);
                //this.setBackgroundDrawable(getResources().getDrawable(R.drawable.button));
            }{
                if(mFacebookBtn){
                    this.setBackgroundDrawable(getResources().getDrawable(R.drawable.button));
                }else {
                    this.setBackgroundDrawable(getResources().getDrawable(R.drawable.boder));
                }
            }
        }
    }
    ...
    public void setButtonBackground(boolean background){
